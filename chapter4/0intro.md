...
本章将进一步设计与实现具有上述大部分功能的侏罗纪“头甲龙” <sup>[1](#tutus)</sup> 操作系统，让应用开发更加简单，应用程序更加通用，且让应用和操作系统都有强大的地址空间隔离的安全保护。

<a id="term-illusion"></a>

<a id="term-time-division-multiplexing"></a>

<a id="term-transparent"></a>
...
![头甲龙操作系统 - Address Space OS总体结构](./addr-space-os-detail.png)
...
由于代表应用程序运行的任务和管理应用的操作系统各自有独立的页表和虚拟地址空间，所以在操作系统的设计实现上需要考虑两个挑战。第一个挑战是 **页表切换** 。由于系统调用、中断或异常导致的应用程序和操作系统之间的上下文切换不像以前那么简单了，因为在这些处理过程中需要切换页表，相关改进可参看  `os/src/trap/trap.S` 。还有就是需要对来自用户态和内核态的异常/中断分别进行处理，相关改进可参看  `os/src/trap/mod.rs` 和  [跳板的实现](6multitasking-based-on-as.md#term-trampoline) 中的讲解。
