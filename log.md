...
- 旧版的 3.5.0 文档及代码（全七章）已经发布在 [这里](https://github.com/rcore-os/rCore-Tutorial-v3/releases/tag/v3.5.0) 。目前开始在主分支上更新新版的文档和代码。

## 2021-03-15

- 增加了在做实验的时候打补丁继承上一章节修改的 [教程](chapter2/5exercise.md#inherit-last-ch-changes) 。
...
- 支持二级间接块索引，使得支持的单个文件最大容量从 $94\text{KiB}$ 变为超过 $8\text{MiB}$ 。调整了单个 `DiskInode` 大小为 128 字节。
...
另外，在所有章节分支新增了 docker 支持来尽可能降低环境配置的时间成本，详见 [使用 Docker 环境](chapter0/5setup-devel-env.md#link-docker-env) 。
