...
之前我们给出过支持应用程序运行的一套 [执行环境栈](../chapter1/1app-ee-platform.md#app-software-stack) ，现在我们站在特权级架构的角度去重新看待它：
...
在特权级相关机制方面，本书正文中我们重点关心 RISC-V 的 S/U 特权级， M 特权级的机制细节则是作为可选内容在 [附录 C：深入机器模式：RustSBI](../appendix-c/index.md) 中讲解，有兴趣的同学可以参考。
...
- 指令访问了 [S模式特权级下才能访问的寄存器](4trap-handling.md#term-s-mod-csr) 或内存，如表示S模式系统状态的 **控制状态寄存器** `sstatus` 等。
...
| 访问 S 模式 CSR 的指令 | 通过访问 [sepc/stvec/scause/sscartch/stval/sstatus/satp等CSR](4trap-handling.md#term-s-mod-csr) 来改变系统状态：在 U 模式下执行会产生非法指令异常 |

在下一节中，我们将看到 [在 U 模式下运行的用户态应用程序](2application.md#term-csr-instr-app) ，如果执行上述 S 模式特权指令，将会产生非法指令异常，从而看出 RISC-V 的特权模式提供了对操作系统一定程度的保护。
